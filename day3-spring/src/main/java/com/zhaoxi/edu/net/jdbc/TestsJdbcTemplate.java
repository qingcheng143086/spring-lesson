...
public class TestsJdbcTemplate {
    ...
    private static void testInsert(JdbcTemplate jdbcTemplate) {
        int row = jdbcTemplate.update("INSERT INTO `bootdb`.`order_1990`(`order_id`, `order_money`, `order_address`) VALUES (222, 3333.00, 'dkd')");
    ...
    private static void testQuery(JdbcTemplate jdbcTemplate) {
        // 把查询的结果转换为对应类型对象集合
        List<Order> query = jdbcTemplate.query("select * from order_1990", new RowMapper<Order>() {
            @Override
